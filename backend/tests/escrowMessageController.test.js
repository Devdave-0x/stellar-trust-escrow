...
describe('escrowMessageController', () => {
      ...
      const messages = [
        {
          id: 2,
          escrowId: 1n,
          senderAddress: FREELANCER_ADDRESS,
          body: 'second',
          readBy: [FREELANCER_ADDRESS],
          createdAt: new Date(),
        },
        {
          id: 1,
  ...
  describe('markRead', () => {
    it('marks all unread messages as read for the requesting participant', async () => {
      ...
      prismaMock.escrowMessage.findMany.mockResolvedValue([
        { id: 1, readBy: [CLIENT_ADDRESS] },
        { id: 2, readBy: [CLIENT_ADDRESS] },
      ]);
      prismaMock.escrowMessage.update.mockResolvedValue({});

      const req = {
        params: { id: '1' },
        user: { address: FREELANCER_ADDRESS },
        tenant: { id: 'test-tenant-id' },
      };
      const res = createMockRes();
      ...
      expect(prismaMock.escrowMessage.update).toHaveBeenCalledWith({
        where: { id: 1 },
        data: { readBy: [CLIENT_ADDRESS, FREELANCER_ADDRESS] },
      });
  ...
  describe('getUnreadCount', () => {
    it('counts unread messages across all of the user escrows', async () => {
      ...
      const req = { user: { address: CLIENT_ADDRESS } };
      const res = createMockRes();
      ...
      expect(prismaMock.escrowMessage.count).toHaveBeenCalledWith({
        where: {
          escrowId: { in: [1n, 2n] },
          NOT: { readBy: { array_contains: CLIENT_ADDRESS } },
        },
      });
      expect(res.body).toEqual({ count: 3 });
    });

    it('returns 0 when the escrow lookup for the address comes back empty', async () => {
      prismaMock.escrow.findMany.mockResolvedValue([]);

      const req = { user: { address: OUTSIDER_ADDRESS } };
      const res = createMockRes();

      await escrowMessageController.getUnreadCount(req, res);

      expect(res.body).toEqual({ count: 0 });
      expect(prismaMock.escrowMessage.count).not.toHaveBeenCalled();
    });
  });
});
