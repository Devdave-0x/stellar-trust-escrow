...
// This sorted stringifier ensures deterministic serialisation for cache keys.
 ...
 * Suspends a user — sets status='suspended' on the User row and logs the action.
 */
const suspendUser = async (req, res) => {
  try {
    ...
    res.json({ message: `User ${address} suspended.`, auditEntry: result.auditEntry });
  } catch (err) {
...
/**
 * POST /api/admin/users/:address/unsuspend
 * Lifts a suspension — restores status='active' on the User row and logs the action.
 */
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (user.status !== 'suspended') {
        return { notSuspended: true, currentStatus: user.status };
      }
    ...
    if (result.notSuspended) {
      return res.status(409).json({
        error: `User is not suspended (current status: ${result.currentStatus}).`,
      });
    }

    await cache.invalidatePrefix(`admin:user:${address}`);
    res.json({ message: `User ${address} unsuspended.`, auditEntry: result.auditEntry });
  } catch (err) {
    logControllerError('admin.unsuspendUser', err, req);
    res.status(500).json({ error: err.message });
  }
};
...
const banUser = async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.user.update({
        where: { id: user.id },
        data: { status: 'banned', suspendedAt: new Date(), suspendReason: reason },
      });
